<template>
  ...
  <div class="table_container bodyClass">
    ...
    <el-form ref="form" :inline="true" :model="searchValue" label-width="80px" class='el-form-search'>
      ...
      <el-form-item label="授信业务类型">
         <el-select v-model="searchValue.projectType" filterable placeholder="请选择">
          <el-option v-for="item in project_types" :key="item.label" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="授信业务金额" class="billDataAll">
        <div class="el-form-cellcon">
          <el-input @keyup.enter.native="search(1)" v-model="searchValue.bussinessMoneyBegin" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"></el-input>
          <span class="block_span">-</span>
          <el-input @keyup.enter.native="search(1)" v-model="searchValue.bussinessMoneyEnd" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"></el-input>
        </div>
      </el-form-item>
    </el-form>
    <!-- 操作按钮 -->
    <el-row class="operationBtn" type="flex" justify="end">
      <el-col :span="20">
        <el-button type="primary" v-if="$shiro.hasPermission('finance:creditAudit:process')" @click="btnOperation('lc')">流程查看</el-button>
        <el-button type="primary" v-if="$shiro.hasPermission('finance:creditAudit:approveAll')"
         @click="btnOperation('ApprovalAll')">批量审批</el-button>
        <el-button type="primary" v-if="$shiro.hasPermission('finance:creditAudit:record')" @click="btnOperation('See')">修改记录</el-button>
        <el-button type="primary" v-if="$shiro.hasPermission('finance:creditAudit:print')"
    ...
    <!-- 表格数据 -->
     <div class="operation_block">
      <el-table ref="table" v-loading="loading" stripe :data="tableData" border style="width: 100%" :height="tableMaxHeight" size="medium"  @selection-change="selectedDel" class="cell_padd" @sort-change='getTableData'>
        ...
        <el-table-column width="120px" align="right" prop="projectType" label="授信业务类型" ></el-table-column>
        <el-table-column width="120px" align="right" prop="bussinessDate" label="授信业务期限" ></el-table-column>
        <el-table-column width="120px" align="right" prop="bussinessMoney" label="授信业务金额" :formatter='formatMoney' header-align='center'></el-table-column>
        <el-table-column width="120px" align="right" prop="applyNuit" label="名义申请单位" ></el-table-column>
    ...
    data() {
      return {
        ...
        searchValue: {
          ...
          print:null,//是否打印
          projectType:"",//授信业务类型 
          bussinessMoneyBegin:'', //授信业务起始金额
          bussinessMoneyEnd:'', //授信业务结束金额
        },
        ...
        pickerOptions:{},
        billType:'SXYW',
        urlType:'/finance/financeCreditBusiness'
      }
    },
    created () {
      let text={
    ...
    methods: {
      ...
      async getTableData(value) {
        ...
        var searchData = {
          ...
          contractCode: this.searchValue.contractCode || "", //合同编号
          contractName: this.searchValue.contractName || "", //合同名称
          billDateBegin: this.searchValue.billDateBegin || "", //单据起始日期
          billDateEnd: this.searchValue.billDateEnd || "", //单据结束日期
          projectType: this.searchValue.projectType || "", //合同编号
          bussinessMoneyBegin: this.searchValue.bussinessMoneyBegin || "", //授信业务起始金额
          bussinessMoneyEnd: this.searchValue.bussinessMoneyEnd || "", //授信业务结束金额
          order:stringSort, //排序
      ...
      btnOperation(str, scopes) {
        ...
        if (str == "Approval") {
          ...
          let title='审批-'+ this.titles;
          this.$util.layerFrame(title, this.$page.SXYW, '95%', '95%', _that)
        }
        //行内查看
        if (str == "Sees") {
          this.modifyId = scopes.id
