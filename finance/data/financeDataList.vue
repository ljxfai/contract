  ...
  <div class="table_container bodyClass">
    ...
    <el-row class="operationBtn" type="flex" justify="end">
      <el-col :span="20">
        ...
        v-if="$shiro.hasPermission('finance:data:add')">增加</el-button>
        <el-button type="primary" @click="btnOperation('modify')" 
        v-if="$shiro.hasPermission('finance:data:edit')">修改</el-button>
        <el-button type="primary" @click="delTableData(1)" 
        v-if="$shiro.hasPermission('finance:data:del')">删除</el-button>
        <el-button type="primary" @click="btnOperation('See')"
         v-if="$shiro.hasPermission('finance:data:view')">查看</el-button>
        <el-button type="primary" @click="btnOperation('lc')"
         v-if="$shiro.hasPermission('finance:data:process')">流程查看</el-button>        
        <el-button type="primary" v-if="$shiro.hasPermission('finance:data:print')">打印</el-button>
        <el-button type="primary" @click="btnOperation('dc')"
         v-if="$shiro.hasPermission('finance:data:export')">导出</el-button>
        <el-button type="primary" @click="btnOperation('invalid')"
         v-if="$shiro.hasPermission('finance:data:invalid')">作废</el-button>
      </el-col>
      <el-col :span="4">
  ...
  export default {
    ...
    methods: {
      ...
      getTableData(value){
        ...
        this.$http.get(`${this.requestURL}/page`,{params:searchData})
        .then((res) => {
          if(res.data.code==0){
      ...
      btnOperation(str, scopes) {
        ...
        if (str == "add") {
          ...
          this.$util.layerFrame(titles, this.$page.ZLJY, '95%', '95%', this)
        }
        //行内修改
        if (str == "modifys") {
          ...
          this.$util.layerFrame(titles, this.$page.ZLJY, '95%', '95%', _that)
        }
        //表头修改
        if (str == "modify") {
