<template>
  ...
  <div class="table_container" id="table_MonthlyBonusForm">
    ...
    <!-- 基本信息 table-->
    <div v-show="active==0" class="circle_content" style="margin-top:30px">
      <div class="tops TabButton tabbtn_list">
        <span class="eltabelHeader-title">资料借阅明细</span>
        <div class="topsBtn">
          <el-button
            class="buttons"
            type="primary"
      ...
      <el-table
        ref="xTable"
        :data="rowData.financeDataBorrowDetails"
        style="width: 100%;"
        height="200px"
        border
      ...
      >
        ...
        <el-table-column prop="fileDataName" align="center" label="资料名称" min-width="260">
          <template slot-scope="scope">
            <el-input
              :class="{'global_writeinput' : isWrite}"
              :readonly="!isWrite"
              style
              class="edit-input left_input"
              type="text"
              v-model="scope.row.fileDataName"
            ></el-input>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="scope">
            <el-select :disabled="!isWrite" size="mini" v-model="scope.row.isnCopy" must="是否原件">
              <el-option
                v-for="item in isnCopyList"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              ></el-option>
        ...
        <el-table-column prop="fileDataCount" align="center" label="附件数量" width="120">
          <template slot-scope="scope">
            <el-input
              :class="{'global_writeinput' : isWrite}"
              :readonly="!isWrite"
              ...
              type="number"
              min="0"
              v-model="scope.row.fileDataCount"
              
            ></el-input>
          </template>
        </el-table-column>
        <!-- 预计归还日期 -->
        <el-table-column prop="returnDate" align="center" label="预计归还日期" width="180">
          <template slot-scope="scope">
            <el-date-picker
        ...
        <!-- <el-table-column prop="focusedDepartName" align="center" label="归口部门"></el-table-column> -->
        <el-table-column prop="memo" align="center" label="备注" min-width="500">
          <template slot-scope="scope">
            <el-input
              :class="{'global_writeinput' : isWrite}"
              :readonly="!isWrite"
    ...
    <!-- 附件 -->
    <div v-show="active==1" class="circle_content" style="margin-top:30px">
...
export default {
  ...
  data() {
    return {
      ...
      requestURL:'/finance/financeDataBorrow',//接口地址
      // rowData.financeDataBorrowDetails: [],//调整明细集合
      contents: [], //调整明细选中数据
      active: 0, //下一步下标
      arr: ["授信明细", "附件上传"], //导航名称
      ...
      isnCopyList:this.$util.getDictData('dict_yes_no'),//是否原件  0否  1是（dict_yes_no）
  ...
  methods: {
    ...
    getData(){
      ...
      this.$http
        ...
        .then(res => {
          ...
          // console.log(this.rowData.operatorName ,"========this.rowData.operatorName ")
          // this.rowData.financeDataBorrowDetails = this.$util.getDictTitle("dict_depart",this.rowData.departId );
          console.log(this.rowData,'----------rowData')
           //附件
          this.attachmentListdata = res.data.data.dateAttachment;     
          
          // this.rowData.financeDataBorrowDetails = res.data.data.financeDataBorrowDetails
          // console.log(this.rowData.financeDataBorrowDetails)
          ...
          // for (let i = 0; i < this.rowData.financeDataBorrowDetails.length; i++) {
          //   this.rowData.financeDataBorrowDetails[i].indexs = i;
          //   //调整明细金额转千分符
          //   if (
          //     this.rowData.financeDataBorrowDetails[i].departId != "" &&
          //     this.rowData.financeDataBorrowDetails[i].departId != null
          //   ) {
          //     this.rowData.financeDataBorrowDetails[
          //       i
          ...
          //     );
          //   } else {
          //     this.rowData.financeDataBorrowDetails[i].departName = "";
          //   }
          //   this.rowData.financeDataBorrowDetails[i].moneyBefore = this.$util.fmtMoney(
          //     this.rowData.financeDataBorrowDetails[i].moneyBefore,
          //     2
          //   );
          //   this.rowData.financeDataBorrowDetails[i].moneyBeforeUsable = this.$util.fmtMoney(
          //     this.rowData.financeDataBorrowDetails[i].moneyBeforeUsable,
          //     2
          //   );
          //   this.rowData.financeDataBorrowDetails[i].moneyAdjust = this.$util.fmtMoney(
          //     this.rowData.financeDataBorrowDetails[i].moneyAdjust,
          //     2
          //   );
          //   this.rowData.financeDataBorrowDetails[i].moneyAfter = this.$util.fmtMoney(
          //     this.rowData.financeDataBorrowDetails[i].moneyAfter,
          //     2
          //   );
          //   this.rowData.financeDataBorrowDetails[i].moneyAfterUsable = this.$util.fmtMoney(
          //     this.rowData.financeDataBorrowDetails[i].moneyAfterUsable,
          //     2
          //   );
          // }
          ...
          //调整明细 id转换         
          //默认1行
          if (this.rowData.financeDataBorrowDetails.length == 0) {
            this.Adds(0);
          }
        })
        .catch(err => {
    ...
    Submission(types) {
      ...
      if(this.rowData.financeDataBorrowDetails){
        for (let i = 0; i < tableDatas.financeDataBorrowDetails.length; i++) {
          //金额
          // if (tableDatas.financeDataBorrowDetails[i].amount === "" || tableDatas.financeDataBorrowDetails[i].amount == null
          // ) {
          //   tableDatas.financeDataBorrowDetails[i].amount = "";
          // } else {
          //   tableDatas.financeDataBorrowDetails[i].amount = this.$util.delcommafy(tableDatas.financeDataBorrowDetails[i].amount);
          // }
          if(tableDatas.financeDataBorrowDetails[i].isAudit ==null)tableDatas.financeDataBorrowDetails[i].isAudit=""
        }
      }
    ...
    Adds(n) {
      if(this.rowData.financeDataBorrowDetails==null){
        this.rowData.financeDataBorrowDetails=new Array()
      }
      //新增
      if (n == 0) {
        this.rowData.financeDataBorrowDetails.push({
          id:"" ,//"(string)", 
          check:false,// "(boolean)"
          fileDataCount:null,// "(integer)附件数量"
          fileDataName: "", //"(string)借阅资料名称"
          isAudit:"", 
          isnCopy:null,// "(string)是否原件  0否  1是",
    ...
    removeRow(n) {
      ...
      if (n == 0) {
        var newData = [],
          t = 1;
        for (let i = 0; i < this.rowData.financeDataBorrowDetails.length; i++) {
          if (this.rowData.financeDataBorrowDetails[i].switch != undefined) {
            t = 0;
          }
        }
        if (t) {
        ...
        for (let i = 0; i < this.rowData.financeDataBorrowDetails.length; i++) {
          if (this.rowData.financeDataBorrowDetails[i].switch != false) {
            newData.push(this.rowData.financeDataBorrowDetails[i]);
          }
        }
        this.rowData.financeDataBorrowDetails = newData;
        // this.monsyA()
        for (let i = 0; i < this.rowData.financeDataBorrowDetails.length; i++) {
          this.rowData.financeDataBorrowDetails[i].indexs = i;
        }
    ...
    copy(n) {
      ...
      if (n == 0) {
        ...
        for (let i = 0; i < newContents.length; i++) {
          this.rowData.financeDataBorrowDetails.push(newContents[i]);
        }
        for (let i = 0; i < this.rowData.financeDataBorrowDetails.length; i++) {
          this.rowData.financeDataBorrowDetails[i].indexs = i;
        }
        ...
        _this.$http
          .get(url, {
            params: {
              ...
              documentsType: "ZT",
              projectId:
                _this.rowData.financeDataBorrowDetails[_this.remoteMethodIndexs].projectId
            }
          })
          .then(function(res) {
            if (res.data.code == 0) {
    ...
    budgetFront(d) {
      ...
      for (let i = 0; i < this.rowData.financeDataBorrowDetails.length; i++) {
        if (
          ...
          this.rowData.financeDataBorrowDetails[i].budgetDepartId != "" &&
          this.rowData.financeDataBorrowDetails[i].budgetDepartId != null &&
          this.rowData.financeDataBorrowDetails[i].budgetSubjectId != "" &&
          this.rowData.financeDataBorrowDetails[i].budgetSubjectId != null
        ) {
          datas = {
            projectId: this.rowData.projectId,
            budgetDepartId: this.rowData.financeDataBorrowDetails[i].budgetDepartId,
            budgetSubjectId: this.rowData.financeDataBorrowDetails[i].budgetSubjectId,
      ...
      this.$http.post("budget/execute/getMoneySumBudgets", newArr).then(res => {
        if (res.data.code != 1) {
            ...
            for (let q = 0; q < res.data.data.length; q++) {
              if (
                ...
                if (
                  ...
                  this.rowData.financeDataBorrowDetails[i].moneyBefore = 0.0;
                  this.rowData.financeDataBorrowDetails[i].moneyBeforeUsable = 0.0;
                } else {
                  this.rowData.financeDataBorrowDetails[i].moneyBefore = this.$util.fmtMoney(
                    res.data.data[q].moneySumBudget,
                    2
          ...
          // this.rowData.financeDataBorrowDetails[i].moneyBefore = this.$util.fmtMoney(res.data.data.moneySumBudget, 2)
          // this.rowData.financeDataBorrowDetails[i].moneyBeforeUsable = this.$util.fmtMoney(res.data.data.moneyUsable, 2)
        }
      });
    },
    // 财政资金格式化金额
    ...
    moneyTotal(row, t) {
      ...
      for (let i = 0; i < this.rowData.financeDataBorrowDetails.length; i++) {
        this.rowData.financeDataBorrowDetails[i].moneyAfter = this.$util.fmtMoney(
          this.$util.delcommafy(this.rowData.financeDataBorrowDetails[i].moneyBefore) +
            this.$util.delcommafy(this.rowData.financeDataBorrowDetails[i].moneyAdjust),
          2
        );
        this.rowData.financeDataBorrowDetails[i].moneyAfterUsable = this.$util.fmtMoney(
          this.$util.delcommafy(this.rowData.financeDataBorrowDetails[i].moneyBeforeUsable) +
            this.$util.delcommafy(this.rowData.financeDataBorrowDetails[i].moneyAdjust),
          2
        );
      }
      this.$set(
        this.rowData,
        "moneyBudget",
        this.$util.moneyAll(this.rowData.financeDataBorrowDetails, "moneyAdjust")
      );
    },
    //表格金额去除格式化
    moneyFocus(row, t) {
    ...
    selectAll(selection) {
      ...
      if (selection != 0) {
        ...
        for (let i = 0; i < this.rowData.financeDataBorrowDetails.length; i++) {
          this.rowData.financeDataBorrowDetails[i].switch = true;
        }
      }
    },
    MonthlyBonusForm(content) {
    ...
    busiIdSuccess(index) {
      for (let i = 0; i < this.busiIdOptions.length; i++) {
        if (
          this.rowData.financeDataBorrowDetails[index].busiName == this.busiIdOptions[i].value
        ) {
          //业务事项id
          this.rowData.financeDataBorrowDetails[index].busiId = this.busiIdOptions[i].value;
          //业务事项名称
          this.rowData.financeDataBorrowDetails[index].busiName = this.busiIdOptions[i].name;
          //预算科目id
          this.rowData.financeDataBorrowDetails[index].budgetSubjectId = this.busiIdOptions[
            i
          ].budgetSubjectId;
          //预算科目名称
          this.rowData.financeDataBorrowDetails[index].budgetSubjectName = this.busiIdOptions[
            i
          ].budgetSubjectName;
          //归口部门id
          this.rowData.financeDataBorrowDetails[index].focusedDepartId = this.busiIdOptions[
            i
          ].budgetSubjectDepartid;
          //归口部门名称
          this.rowData.financeDataBorrowDetails[index].focusedDepartName = this.busiIdOptions[
            i
          ].budgetSubjectDepart;
        }
      }
    ...
    budgetDepartnamesuccess(index) {
      for (let i = 0; i < this.budgetDepartnameoptions.length; i++) {
        if (
          this.rowData.financeDataBorrowDetails[index].budgetDepartname ==
